...
- **Ubuntu 22.04, not 20.04.** RustiFlow's ring buffers need kernel ≥ 5.8; 22.04's
  stock kernel is 5.15, so there is **no manual HWE-install + reboot step**. If
  `UBUNTU22-64-STD` doesn't resolve on wall2, fall back to `UBUNTU20-64-STD`, add
  `sudo apt-get install -y linux-generic-hwe-20.04` to `bootstrap.sh`, and reboot
  once before provisioning (the wart this design exists to avoid).
